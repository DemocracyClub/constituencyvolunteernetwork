...
"""def callback_assign_all(sender, **kwargs):
        print "Signal received %s" % kwargs['user']
        user = kwargs['user']
        task = Task.objects.get(slug="gather-candidate-information")
        ...
        try: # Make sure we're not already assigned this task, unless we want to
            ...
            url = "http://www.blah.com/%s" % user.email
...
def assign_to_all(task_slug, url_template):
    ...
    user_join.connect(callback_assign_all)
    user_touch.connect(callback_assign_all)
    
    return callback_assign_all
